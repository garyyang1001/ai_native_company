...
4.  **Phase 4: Gary Gate UI (interactive action 已開始)**
    *   已落地：HTTP routes、本地 approve/reject action、`http_app` serve/seed/seed-and-serve subcommand 拆分（2026-05-24 完成，serve 預設不 reset、保留歷史）。
    *   待辦：`views.py` / `http_app.py` 加上 `sql_patch` 顯示路徑（含 sandbox_schema、replay rows 摘要），讓 UI 能反映 SQL self-healing scenario 跑完後的狀態。
