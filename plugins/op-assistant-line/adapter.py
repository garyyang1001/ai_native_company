...
class LineAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        platform = Platform("op-assistant-line")
        ...
        default_prefixes = ("小弟", "@小弟", "/小弟")
        configured = os.environ.get("OP_INVOCATION_PREFIXES", "").strip()
        if configured:
            self._invocation_prefixes = tuple(
                p.strip() for p in configured.split(",") if p.strip()
            )
        else:
            self._invocation_prefixes = default_prefixes
    ...
    async def _handle_health(self, request) -> Any:
        from aiohttp import web
        return web.json_response({"status": "ok", "platform": "op-assistant-line"})
    ...
    async def _handle_message_event(self, event: Dict[str, Any]) -> None:
        ...
        # OP-Assistant fork additions:
        #   1. Always log inbound to closed_loop_kernel.events for learning.
        #   2. Python-deterministic invocation filter — Code is Law.
        # Hermes's bundled LINE plugin lacks require_mention support. We gate
        # here so silent observations are recorded but do not spawn agent turns.
        try:
            self._log_inbound_to_kernel(event_obj, msg, chat_type, source)
        except Exception as exc:
            # NEVER block the message pipeline because of audit failure.
            logger.warning("op-assistant-line: kernel audit log failed: %s", exc)

        invocation = self._detect_invocation(text, msg)
        if invocation is None:
            logger.debug(
                "op-assistant-line: silent observe (no invocation), msg_id=%s",
                message_id,
            )
            return
        ...
        logger.info(
            "op-assistant-line: invoked via %s, msg_id=%s",
            invocation["kind"],
            message_id,
        )
    ...
    def _detect_invocation(self, text: str, msg: dict) -> dict | None:
        ...
        Returns a structured mention or prefix descriptor when invoked.
        Silent observations return None.
        """
        mention = msg.get("mention") if isinstance(msg, dict) else None
        if isinstance(mention, dict):
            for m in mention.get("mentionees") or []:
                if isinstance(m, dict) and m.get("isSelf") is True:
                    ...
                    return {"kind": "mention", "index": idx, "length": length}

        text_stripped = (text or "").strip()
        for prefix in self._invocation_prefixes:
    ...
    def _strip_invocation_token(self, text: str, invocation: dict) -> str:
        """Return text with the invocation token removed."""
        if invocation["kind"] == "mention":
            idx = invocation.get("index", 0)
            length = invocation.get("length", 0)
            if length > 0:
    ...
    def _log_inbound_to_kernel(
        ...
        """Always write an inbound event to closed_loop_kernel for learning."""
        import sys
        from datetime import datetime, timezone
        ...
        if not kernel_url:
            logger.debug(
                "op-assistant-line: KERNEL_DATABASE_URL not set; skipping kernel audit"
            )
            return

        inbound_namespace = uuid.UUID("a1b2c3d4-0000-0000-0000-000000000006")
        msg_id = event_obj.message_id or "no-msg-id"
        event_id = str(uuid.uuid5(inbound_namespace, msg_id))
        ...
        payload = {
            ...
            "user_id": source.get("userId", ""),
            "mention_is_self": any(
                isinstance(m, dict) and m.get("isSelf") is True
                for m in ((msg.get("mention") or {}).get("mentionees") or [])
            )
            if isinstance(msg.get("mention"), dict)
            else False,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        name="op-assistant-line",
        label="LINE (op-assistant)",
