...
| R18 | **failures → inbound event 鏈穩定性**(Round 2 codex assumption #2)| **Round 3 spike DONE**:production query 證實 chain 斷:`attempts.event_id` 全 NULL,`attempt_envelopes.source_refs` UUID 不對齊 events 表 → V0.3 降階用 substring fallback(confidence 0.5);V0.4 修 V0.2 adapter(見 R19) |
| R19 | V0.2 LINE adapter 沒填 `attempts.event_id`,`attempt_envelopes.source_refs` 用了跟 events.id 不同的 uuid5 seed,production chain 完全斷 | V0.3 接受降階(`candidate_sources` 全 substring_match, confidence 0.5);V0.4 改 `plugins/op-assistant-line/adapter.py _op_write_pg`:寫 attempt 時帶 inbound event_id + source_refs reuse events.id;historical row backfill 困難可接受 |
...
**Round 3**(`round-03.md`):
- R18 spike(無 codex consult,pure factual investigation):production op_assistant_kernel query 證實 `attempts.event_id` NULL + `attempt_envelopes.source_refs` UUID 不對齊 events 表
- Phase 2 U39 candidate_sources 降階決策:`source_type='substring_match'` confidence 0.5(V0.4 修 V0.2 adapter,加 R19 追蹤)
- Phase 2 開工 ready
