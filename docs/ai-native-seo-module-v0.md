...
```text
approved Hermes profile template
  -> copy model / provider baseline
  -> rewrite SOUL.md for the exact SEO role
  -> restrict tools and cwd for that role
  -> require Agent Output Envelope
  -> import traces into Closed Loop Kernel
```

白話說：新 OHYA SEO Agent 不另外挑模型，也不直接沿用舊 OHYA 的靈魂檔、記憶、sessions 或髒任務。它只沿用乾淨 Hermes profile 的設定格式與模型基準，角色內容重新寫。
...
第一批不建立全部 profiles。建議先做三個可驗證 profile：

```text
growth-coordinator
  白話：接任務、建 Task Record、派工，不自己做 SEO 判斷。

social-listener
  白話：只做公開社群海巡，產出 social_patrol_report / brand_presence_signal。

social-reply-advisor
  白話：只產生 social_reply_recommendation，不發布、不傳送。
```

這三個跑通後，再加入 `seo-content-strategist`、`reviewer`、`social-operator` 與 `outcome-monitor`。

驗收標準：

1. 每個 profile 都有乾淨 `SOUL.md` 與 `config.yaml`。
2. `config.yaml` 的模型基準與 approved Hermes template 相同。
3. 每個 profile 的輸出都能被 `data/agent-profile-registry-v0.json` 驗證。
4. 不複製舊 OHYA memories、sessions、logs、credentials 或 kanban state。
5. 不寫回 live OHYA runtime；先用 isolated scaffold / snapshot 驗證。
